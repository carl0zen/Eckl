...
<script type="text/javascript">
	...
	function delete_picture(obj,hash){
		...
		$.ajax({
			type: "POST",
			url: "<?php echo _ROOT_PATH_URL_?>/include/members/gallery/actions.php?q="+ 1*new Date(),
	...
	#gallery #picture{
		background:url(<?php echo _ROOT_PATH_URL_?>/images/ajax-loader.gif) no-repeat center;
		width:78px;
		height:59px;
		overflow:hidden;
		background:#ddd;
...
<div id="loadpics" onClick="javascript:$(location).attr('href','<?php echo _ROOT_PATH_URL_;?>/template.php?goto=image-uploader');">Subir Imagenes</div>
<?php } ?>
...
if(mysql_num_rows($rst)){
	...
	while($row=mysql_fetch_assoc($rst)){
		...
		if(file_exists($image_path) && file_exists($image_path_th)){?>
			<div id="picture"><div onclick="javascript:delete_picture($(this),'<?php echo $row['hash']?>');" id="delete"><span class="ui-icon ui-icon-circle-close"></span><div class="text">Delete</div></div><a  onClick="javascript: current_gallery_page=<?php echo $no_page?>; load_html('content', '<?php echo _ROOT_PATH_URL_?>/include/members/gallery/show_picture.php?no_page=<?php echo ($no_page*$pictures_page)+$c;?>&user_id=<?php echo $user_id?>')" href="javascript:;"><img src="<?php echo $image_url_th;?>"></a></div>
...
?>
	...
	<div id="pager"><div>Paginas:</div>
		...
		?>
	    <div onClick="javascript: load_html('content', '<?php echo _ROOT_PATH_URL_?>/include/members/gallery/gallery.php?no_page=<?php echo $p;?>&user_id=<?php echo $user_id?>')" ><a href="javascript:;"><?php echo $p+1?></a></div>
        <?php $p++;	}?>
    <?php }else{?>
