...
<script type="text/javascript" src="<?php echo _ROOT_URL_;?>/embed/jquery.oembed.1.1.0/jquery.oembed.js"></script>

<script src="<?php echo _ROOT_URL_?>/js/jquery.timeago/jquery.timeago.js"></script>
<script src="<?php echo _ROOT_URL_?>/js/jquery.timeago/jquery.timeago.es.js"></script>
	...
	if(mysql_num_rows($rst_stream_messages)){
		while($row_stream_messages=mysql_fetch_assoc($rst_stream_messages)){
			if(file_exists($GEN_PATH_MEMBERS_PICTURES.members_get_info("hash",$row_stream_messages['from_user_id'])."/profile_mini.jpg")){
				...
				$image_url_profile_mini=_ROOT_URL_."/images/avatar_mini.png";
			}
			?>
            <div id="comments_list_<?php echo $row_stream_messages['members_stream_id']?>">
	            <div id="comments">
    		        <div class="comment" id="comment_<?php echo $row_stream_messages['members_stream_id']?>">
                        ...
                        <div class="avatar"><a href="<?php echo _ROOT_URL_."/profile.php?user_id=".$row_stream_messages['from_user_id'];?>"><img src="<?php echo $image_url_profile_mini;?>"></a>
                            <div class="category"><div id="category_id_<?php echo $row_stream_messages['skill_category_id'];?>"></div></div>
                        </div>
                        <div class="comment_text">
                            <div id="member_link"><a href="<?php echo _ROOT_URL_."/profile.php?user_id=".$row_stream_messages['from_user_id'];?>"><?php echo members_get_info("nombre",$row_stream_messages['from_user_id']) . " " . members_get_info("apellido",$row_stream_messages['from_user_id'])?></a>&nbsp;<span id="timestamp"><abbr class="timeago" title="<?php echo $row_stream_messages['date2']?>"><?php echo $row_stream_messages['date2']?></abbr></span></div>
...
//				members_stream where members_stream_parent_id =".$row_stream_messages['members_stream_id'] ." and members_stream_parent_id>0 $limit";
				...
				while($row_sub_comments=mysql_fetch_assoc($rst_stream_sub_messages)){
					if(file_exists($GEN_PATH_MEMBERS_PICTURES.members_get_info("hash",$row_sub_comments['from_user_id'])."/profile_mini2.jpg")){
						...
						$image_url_profile_mini=_ROOT_URL_."/images/avatar_mini2.png";
					}
