...
<div id="loadpics" onClick="javascript:$(location).attr('href','<?php echo $GEN_URL_SERVIDOR;?>/template.php?goto=image-uploader');">Subir Imagenes</div>
<?php } ?>
...
if(mysql_num_rows($rst)){
	...
	while($row=mysql_fetch_assoc($rst)){
		...
		if(file_exists($image_path) && file_exists($image_path_th)){?>
			<div id="picture"><div onclick="javascript:delete_picture($(this),'<?php echo $row['hash']?>');" id="delete"><span class="ui-icon ui-icon-circle-close"></span><div class="text">Delete</div></div><a  onClick="javascript: current_gallery_page=<?php echo $no_page?>; load_html('content', '<?php echo $GEN_URL_SERVIDOR?>/include/members/gallery/show_picture.php?no_page=<?php echo ($no_page*$pictures_page)+$c;?>&user_id=<?php echo $user_id?>')" href="javascript:;"><img src="<?php echo $image_url_th;?>"></a></div>
...
?>
	...
	<div id="pager"><div>Paginas:</div>
		...
		?>
	    <div onClick="javascript: load_html('content', '<?php echo $GEN_URL_SERVIDOR?>/include/members/gallery/gallery.php?no_page=<?php echo $p;?>&user_id=<?php echo $user_id?>')" ><a href="javascript:;"><?php echo $p+1?></a></div>
        <?php $p++;	}?>
    <?php }else{?>
